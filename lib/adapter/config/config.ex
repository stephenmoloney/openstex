defmodule Openstex.Adapter.Config do
  ...
  defmacro __using__(_) do
    quote([]) do
      ...
      def start_agent(_client), do: raise "Openstex.Adapter.Config.start_agent/2 has not been implemented."

      @doc "Starts an agent for the storage of credentials in memory"
      def start_agent(_client, _opts), do: raise "Openstex.Adapter.Config.start_agent/2 has not been implemented."
      ...
      def get_keystone_config_from_env(client, otp_app) do
        get_config_from_env(client, otp_app) |> Keyword.fetch!(:keystone)
      end

      @doc "Gets the swift config.exs environment variables"
      def get_swift_config_from_env(client, otp_app) do
        get_config_from_env(client, otp_app) |> Keyword.fetch!(:swift)
      end
      ...
      def set_account_temp_url_key1(client, key) do
        Agent.update(agent_name(client),
          fn(config) -> put_account_key1(config, key) end
        )
      end
      ...
      def set_account_temp_url_key2(client, key) do
        Agent.update(agent_name(client),
          fn(config) -> put_account_key2(config, key) end
        )
      end
  ...
  @callback start_agent(client :: atom) :: {:ok, pid} | {:error, :already_started}
  @callback start_agent(client :: atom, opts :: Keyword.t) :: {:ok, pid} | {:error, :already_started}
  @callback config(client :: atom) :: Keyword.t | no_return
  @callback keystone_config(client :: atom) :: Keyword.t
  @callback swift_config(client :: atom) :: Keyword.t
  @callback httpoison_config(client :: atom) :: Keyword.t
  @callback tenant_id(client :: atom) :: Keyword.t
  @callback user_id(client :: atom) :: Keyword.t
  @callback keystone_endpoint(client :: atom) :: Keyword.t
  @callback swift_region(client :: atom) :: Keyword.t
  @callback swift_service_name() :: String.t
  @callback swift_service_type() :: String.t
  @callback get_account_temp_url_key1(client :: atom) :: Keyword.t
  @callback get_account_temp_url_key2(client :: atom) :: Keyword.t
  @callback set_account_temp_url_key1(client :: atom, key :: String.t) :: Keyword.t
  @callback set_account_temp_url_key2(client :: atom, key :: String.t) :: Keyword.t
