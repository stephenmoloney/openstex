defmodule Openstex.Services.Keystone.V2.Helpers do
  @moduledoc ~s"""
  ...
  """
  @spec authenticate(String.t, String.t, String.t, Keyword.t) :: {:ok, Identity.t} | {:error, Openstex.Response.t} | {:error, any}
  def authenticate(endpoint, username, password, tenant) do
    Og.context(__ENV__, :debug)

    token_query = Openstex.Services.Keystone.V2.Query.get_token(endpoint, username, password)
    identity_query = fn(token, endpoint, tenant) -> Openstex.Services.Keystone.V2.Query.get_identity(token, endpoint, tenant) end

    with {:ok, resp} <- Request.request(token_query, [], :nil),
         token = resp.body |> Map.get("access") |> Map.get("token") |> Map.get("id"),
         {:ok, resp} <- Request.request(identity_query.(token, endpoint, tenant), [], :nil) do
      {:ok, parse_nested_map_into_identity_struct(resp.body)}
    end
    |> case do
  ...
  def authenticate(endpoint, token, tenant) do
    Og.context(__ENV__, :debug)
    identity_query = fn(token, endpoint, tenant) -> Openstex.Services.Keystone.V2.Query.get_identity(token, endpoint, tenant) end
    case Request.request(identity_query.(token, endpoint, tenant), [], :nil) do
      {:ok, resp} -> {:ok, parse_nested_map_into_identity_struct(resp.body)}
      {:error, resp} -> {:error, resp}
    end
    |> case do
      {:ok, identity} -> {:ok, identity}
