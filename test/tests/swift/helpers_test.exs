defmodule Openstex.Swift.V1.HelpersTest do
  ...
  test "default_hackney_opts()" do
    expected = [timeout: 20_000, recv_timeout: 180_000]
    actual = Swift.client() |> Swift.client().config().hackney_config()
  ...
  test "get_account_tempurl_key() - 1" do
    ...
    actual = Swift.get_account_tempurl_key()
    assert expected == actual
  ...
  test "get_account_tempurl_key() - 2" do
    ...
    Bypass.expect(bypass, fn(conn) ->
      conn
      |> Conn.put_resp_header("x-account-meta-temp-url-key", "server_acquired_temp_url_key1")
      |> Conn.resp(204, "")
    end)
  ...
  test "get_account_tempurl_key() - 3" do
    ...
    actual = Swift.get_account_tempurl_key(:key2)
    assert expected == actual
  ...
  test "get_account_tempurl_key() - 4" do
    ...
    Bypass.expect(bypass, fn(conn) ->
      conn
      |> Conn.put_resp_header("x-account-meta-temp-url-key-2", "server_acquired_temp_url_key2")
      |> Conn.resp(204, "")
    end)
...
#    end)
#
#    Swift.set_account_temp_url_key(:key2, expected)
#    Bypass.down(bypass)
#  end
  ...
  test "upload_file(file, server_object, container, upload_opts \\ [])" do
    ...
    Bypass.expect(bypass, fn(conn) ->
      ...
      conn
      |> Conn.put_resp_header("etag", expected_etag)
      |> Conn.resp(201, "")
    end)

    {:ok, conn} = Swift.upload_file(source_file, "test_object", "test_container")
  ...
  test "download_file(server_object, container)" do
    ...
    Bypass.expect(bypass, fn(conn) ->
      ...
      conn
      |> Conn.put_resp_header("etag", expected_etag)
      |> Conn.put_resp_header("content-length", expected_bytes)
      |> Conn.resp(200, expected_binary)
    end)

    {:ok, conn} = Swift.download_file("test_object", "test_container")
  ...
  test "list_containers()" do
    ...
    {:ok, actual_list} = Swift.list_containers()
  ...
  test "list_objects(pseudofolder, container, [nested: :true])" do
    ...
    {:ok, actual_list} = Swift.list_objects("", "test_container", [nested: :true])
  ...
  test "list_objects(pseudofolder, container, [nested: :false])" do
    ...
    {:ok, actual_list} = Swift.list_objects("", "test_container", [nested: :false])
  ...
  test "list_pseudofolders(pseudofolder, container, opts)" do
    ...
    {:ok, actual_list} = Swift.list_pseudofolders("", "test_container", [nested: :false])
  ...
  test "pseudofolder_exists?(pseudofolder, container)" do
    ...
    actual = Swift.pseudofolder_exists?("nested_folder", "test_container")
    expected = :true
    assert expected == actual
...
#    Bypass.down(bypass)
#
#    Swift.delete_pseudofolder("nested_folder", "test_container")
#    expected = :false
#
...
#
#    Bypass.expect(bypass, fn(conn) ->
#      Conn.put_resp_header(conn, "x-account-meta-temp-url-key", "server_acquired_temp_url_key1")
#      |> Conn.resp(204, "")
#    end)
#
#    # Testing default opts
#    actual_temp_url = Swift.generate_temp_url("test_container", "test_object.json")
...
#    actual_temp_url = Swift.generate_temp_url("test_container", "test_object.json", opts)
#    parsed_url = URI.parse(actual_temp_url)
#    qs_map = URI.decode_query(parsed_url.query)
#
#    assert parsed_url.host == expected_host
